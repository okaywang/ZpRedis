...
namespace StackExchange.Redis.Tests
{
    ...
    public class Cluster : TestBase
    {
        ...
        private const string ClusterIp = "172.17.1.70";   // kmontrose
        private const int ServerCount = 6, FirstPort = 10001;
        ...
        protected override string GetConfiguration()
        {
            ...
            select server + ":" + port) + ",connectTimeout=1000";
        }
        ...
        public void Test1()
        {
            var keyPrefix = System.Threading.Thread.CurrentThread.Name + "_";
            //using (var conn = Create())
            {
                var db = conn.GetDatabase();
                ...
                {
                    //db.StringSet(keyPrefix + i.ToString(), i.ToString());
                    var task = db.StringSetAsync(keyPrefix + i.ToString(), i.ToString());                    
                }
